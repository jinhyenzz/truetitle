...
// 지원 범위: v.daum.net 기사 뷰어 페이지만 (목록/검색 제외). content.ts matches,
// wxt.config.ts host_permissions가 전부 이 상수 하나에서 나와야 어긋나지 않는다.
...
// 광고·공감·관련기사·댓글은 모두 .article_view의 형제 요소라, .article_view만
// 스코프로 잡으면 별도 제외 목록 없이도 섞이지 않는다.
export function extractDaumArticle(doc: Document): ExtractResult {
  ...
  // 원본 DOM을 건드리지 않도록 복제본에서 사진/캡션 등 본문 아닌 요소를 제거.
