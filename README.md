...
네이버·다음 뉴스 기사에서 제목 위 버튼이나 확장프로그램 팝업의 **분석하기**를 누르면 결과를 볼 수 있습니다. 다른 한국어 언론사 기사는 팝업에서 해당 사이트의 접근 권한을 허용한 뒤 추출을 시도합니다. 분석 API는 Railway에 배포되어 있어 로컬 서버 없이도 사용할 수 있으며, Chrome 웹 스토어 등록은 아직 하지 않았습니다.

기사의 사실 여부를 판정하는 서비스는 아닙니다. 결과는 제목을 한 번 더 살펴보기 위한 참고 정보입니다.

## 주요 기능

- 네이버·다음 뉴스에서 제목과 본문을 추출하고, 사용자가 허용한 다른 사이트에는 범용 기사 추출기를 사용합니다.
...
기본 지원 범위는 `n.news.naver.com`, `v.daum.net`의 기사입니다. 다른 HTTPS 언론사 사이트는 개별 권한 허용 후 사용하며, 페이지 구조에 따라 추출에 실패할 수 있습니다. 구글 검색·뉴스 목록 자체를 분석하는 기능은 아닙니다. 검색 결과를 클릭해 도착한 언론사 기사에서 사용합니다. 기사 분석은 버튼을 눌렀을 때 시작합니다.
...
현재는 따옴표 잔여 이스케이프를 정리한 데이터로 학습한 1만 건 모델을 사용합니다. 기존 5만 건 모델은 따옴표 모양에 따라 같은 기사의 점수가 크게 뒤집히는 문제가 있어 임시 교체했습니다. 동일 표본에서 두 모델의 종합 성능 비교는 아직 완료하지 않았으며, 1만 건 모델의 최종 채택을 확정한 것은 아닙니다. 아래는 현재 모델의 학습 기록이며, 기존 5만 건 모델의 수치와는 다른 데이터 처리·표본 조건입니다.
...
이 결과는 AI Hub 검증 데이터에서 측정한 값입니다. 실제 네이버 기사에서도 95.60%를 맞힌다는 의미는 아닙니다.

실제 기사를 확인하면서 큰따옴표 모양만 달라져도 점수가 크게 바뀌는 사례를 발견했습니다. 제목을 굽은 따옴표로 바꾸던 API 보정은 제거했습니다. 현재는 학습과 API가 같은 함수로 제목·본문의 잔여 이스케이프와 연속 공백을 정리하며, 따옴표 모양은 보존합니다. 모델 교체는 표기 민감도를 줄이기 위한 조치이고, 과장·호기심 유발 제목 전체의 판단 능력이 검증됐다는 뜻은 아닙니다.
...
- **모델이 긴 본문 전체를 읽지는 못합니다.** 제목과 본문을 합쳐 최대 128토큰을 입력받으므로 뒤쪽의 조건이나 해명을 놓칠 수 있습니다. 두 입력 중 더 긴 쪽부터 자르기 때문에, 본문이 짧고 제목이 매우 긴 경우에는 제목 뒷부분이 함께 잘릴 수 있습니다.
...
실행 모델은 `artifacts/transformer-10000-quote-normalized-v1/`에 있습니다. `model.safetensors`는 약 272MB이며, 파일이 수백 바이트뿐이라면 실제 모델 대신 LFS 포인터만 받은 상태입니다. 코드만 받고 새 모델을 받지 않으면 `/ready`는 503을 반환합니다. 커밋·push할 때 이 폴더의 모델과 토크나이저 파일도 함께 포함해야 합니다.
...
기본 API 주소는 배포된 Railway API(`https://truetitle-production.up.railway.app`)입니다. `apps/extension/.env` 파일이 없어도 이 주소가 쓰이므로 로컬 서버 실행(위 2단계) 없이 바로 3단계로 진행할 수 있습니다. 로컬 API 서버를 대신 쓰려면 `apps/extension/.env`(gitignore 대상, 커밋되지 않음)에 `WXT_API_BASE_URL=http://127.0.0.1:8001`을 설정하고 다시 빌드합니다. Python 코드 변경 후에는 API를 재시작하고, 확장프로그램을 다시 빌드한 뒤에는 Chrome에서 확장과 기사 탭을 새로고침합니다.
...
응답에는 `clickbait_score`, `clickbait_signal_level`, `clickbait_signal_label`, `classification`, `title_body_similarity`, `evidence`, `model`, `note`가 포함됩니다. 현재 모델 식별자는 `klue-roberta-small-10000-quote-normalized-v1`입니다. 모델·Python 코드를 받은 뒤에는 API 서버를 재시작해야 새 모델이 적용됩니다.
