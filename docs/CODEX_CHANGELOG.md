...
## 2026-09-20 - README 현재 기능 및 실행 안내 정리

- 요청 및 승인: 사용자가 과장되거나 기계적인 문구를 피한 README 정리를 승인함. 역할은 팀원별 한 줄로 간단히 설명함.
- 백업: `.codex-backups/20260920-172002/`에 `README.md`와 이 변경 기록을 원래 경로대로 복사하고 SHA-256 일치를 확인함. `docs/CODEX_WORK_RULES.md`는 존재하지 않아 루트 `AGENTS.md`의 절차를 적용함.
- `README.md`: 프로젝트 목적과 사용 흐름, 실제 기술 및 역할, 저장된 5만 건 모델의 평가 수치와 한계, Git LFS 모델 수신, API 실행, 확장 빌드·Chrome 로드, 검사 명령을 정리함. CI 미구성이라는 오래된 설명과 현재 코드에 없는 본문 전송 20,000자 상한을 수정함. 현재 네이버 지원 범위, 5구간 표시의 의미, 표현 비교와 모델 판단의 차이를 설명함.
- 이유와 영향: 처음 보는 사람이 구현 범위와 실행 방법을 파악할 수 있도록 문서를 갱신함. 수정 범위는 README와 이 기록이며 실행 코드·모델·의존성·CI 설정은 변경하지 않음.
- 검증: API·스키마·모델 추론·요청 제한·확장 추출 및 통신·패키지 스크립트·CI 설정을 읽어 설명과 명령을 대조함. 모델 저장 지표의 학습 50,000건·검증 5,000건·3에포크·정확도 0.9606을 확인함. 문서의 코드 블록 닫힘, 상대 경로 존재, `git diff --check`를 확인함.
- 미검증: 이번 문서 작업에서는 패키지 설치, 새 환경에서의 실행, 모델 재평가, API 테스트 및 Chrome 재검증을 수행하지 않음. Git commit·push·PR·병합은 수행하지 않음.
- 원상복구: 별도 승인 후 위 백업의 README와 변경 기록을 원래 위치에 복원할 수 있음.

## 2026-09-19 - CI API 테스트 도구 누락 복구

- 요청 및 승인: PR #8 병합 뒤 GitHub Actions의 API 테스트가 `No module named pytest`로 실패한 것을 확인했고, 사용자가 수정을 승인함.
- 백업: `.codex-backups/20260919-204649/`에 `services/api/requirements-dev.txt`와 이 변경 기록을 원래 경로대로 복사하고 SHA-256 일치를 확인함.
- 수정: API 개발·CI 의존성에 `pytest==8.3.5`를 추가함. CI가 실행하는 `python -m pytest` 명령의 테스트 도구를 명시적으로 설치하도록 함.
- 영향: 개발·CI 환경에만 적용되며 API 요청/응답, 모델 가중치, 확장 프로그램에는 영향이 없음.
- 검증: 로컬 개발 환경은 unittest를 사용하므로 GitHub Actions와 같은 새 의존성 설치는 실행하지 않음. PR에서 CI가 다시 실행된 뒤 두 작업의 통과 여부를 확인해야 함.
- 원상복구: 별도 승인 후 위 백업의 두 파일을 복원하면 이전 상태로 돌아갈 수 있음.

## 2026-09-19 - 병합 충돌 후 모델 라벨 인덱스 복구

- 요청 및 승인: PR 병합 충돌 해결 과정에서 `predictor.py`의 모델 반환값 계약이 어긋난 것을 확인했고, 사용자가 수정을 승인함.
- 백업: `.codex-backups/20260919-203536/`에 `services/api/app/ml/predictor.py`, `services/api/tests/test_predictor.py`, 이 변경 기록을 원래 경로대로 복사하고 SHA-256 일치를 확인함.
- 수정: `predict_clickbait_score`가 `load_model`의 세 반환값(토크나이저, 모델, 낚시성 라벨 인덱스)을 모두 받고, 하드코딩한 0번 대신 모델 설정의 낚시성 라벨 인덱스로 점수를 읽도록 복구함. 긴 제목 요청을 허용하는 기존 `truncation=True` 설정은 유지함.
- 테스트: `test_predictor.py`에 모델 설정의 낚시성 라벨 인덱스가 1번일 때 해당 점수를 사용하는 회귀 검사를 추가함. API 전체 unittest 36건과 `git diff --check`를 통과함. 실제 로컬 모델을 불러오는 분석·긴 제목 API 검사도 포함됨. Starlette/httpx 사용 중단 예정 경고는 남아 있으나 실패는 아님.
- 영향: 모델 가중치·API 요청/응답 형식·확장 프로그램에는 변경이 없으며, 분석 요청 시 발생할 수 있던 반환값 개수 오류와 라벨 순서 의존을 제거함.
- 원상복구: 별도 승인 후 위 백업의 세 파일을 복원하면 수정 전 상태로 돌아갈 수 있음.
