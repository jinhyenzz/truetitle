...
## 2026-09-20 - 최신 main 통합 및 모델 입력 수정 공유

- 요청 및 승인: 사용자가 소현이의 최신 main 변경을 고려한 통합을 요청하고 "깃까지 진행해줘 같이"로 백업·병합·검증·커밋·푸시를 승인함. 대량 모델 비교와 재학습은 실행하지 않음.
- 백업: `.codex-backups/20260920-210627/`에 미커밋 파일 및 신규 모델을 포함한 22개 파일을 원래 경로로 복사하고 각각 SHA-256 일치를 확인함. 추적 파일 변경은 `codex pre-main-sync 20260920-210627` Git stash에도 보존함. stash는 적용 후 삭제하지 않음.
- 통합: 로컬 main `039c343`에서 원격 main `a015669`로 fast-forward한 뒤 기존 변경을 다시 적용함. 팀원 변경 19개 파일 중 `services/api/app/ml/predictor.py`, `services/api/tests/test_predictor.py`만 충돌함. 한쪽 파일 전체를 선택하지 않고 충돌 구간을 조정함.
- `predictor.py`: 기존 따옴표 강제 변환을 제거하는 로컬 수정과 공통 토큰화 경로를 유지함. 소현이의 홀수 따옴표 보존 의도는 따옴표 모양·짝을 추정하지 않는 공통 정리에서 충족함. 모델 설정의 clickbait 라벨 인덱스 조회를 보존함.
- `test_predictor.py`: 팀원이 추가한 `test_odd_quote_count_is_left_untouched`를 유지하고 `normalize_model_text`를 호출하도록 갱신함. 학습·추론 입력 통일 관련 로컬 테스트도 보존함.
- 팀원 보존 확인: `apps/extension/` 전체와 `services/api/app/core/rate_limit.py`, `services/api/tests/test_rate_limit.py`는 최신 main 대비 차이가 없음. 다음·사이트별 권한·범용 추출·팝업·아이콘 및 IP 처리 변경을 그대로 반영함.
- `README.md`, `docs/test.md`: 최신 사이트 지원 범위와 통합 후 검증 결과를 반영함. `docs/model-evaluation.md`: 현재 공통 절단 규칙과 과거 학습 설정을 구분하고, 중단한 2,000건 모델 비교에서 결과를 얻지 못했음을 명시함. 1만 건 모델은 임시 실행 모델이며 전체 성능 우위·최종 채택을 확정하지 않음.
- 공유 대상: 기존 승인으로 수정한 전처리·학습·평가·추론·테스트·문서와 `app/ml/text.py`, `test_model_inputs.py`, 정리 데이터 1만 건 모델 5개 파일. 모델 safetensors는 Git LFS 대상으로 포함함. 원본 기사 데이터·비밀값·백업·임시 실험 파일은 포함하지 않음. 기존 5만 건 모델도 보존함.
- 검증: CPU 스레드 1개·오프라인 API unittest 45개 전부 통과(생략 없음, 실제 모델 검사 포함). 공백·충돌 표시 검사 통과. Starlette/httpx 사용 중단 예정 경고는 남아 있음.
- 미검증/한계: 새로운 모델의 독립 실전 정확도와 1만/5만 모델 우열은 미확정. 128토큰 이후 본문은 모델에 포함되지 않음. 로컬에는 신규 Readability 의존성이 없어 확장 빌드·Chrome 통합 검사는 실행하지 않음. OFF/초기화 중 응답 표시 문제와 배포 프록시 신뢰 설정은 별도 확인 대상임. 원격 push·CI 결과는 이 기록 작성 시점 이후 별도 확인함.
- 복구: 이후 팀원 변경 여부를 확인하고 승인 아래 이번 공유 커밋만 revert하면 팀원 main 변경은 보존됨. 작업 전 미커밋 상태는 위 백업 및 명명한 stash로 복구할 수 있음. 백업 전체를 최신 main에 무조건 덮어쓰지 않음.

## 2026-09-20 - 평가 실행 오류 수정 및 모델 입력 경로 통일

- 요청 및 승인: 추가 점검 결과와 담당별 수정 범위를 설명한 뒤 사용자가 "진행하자"로 승인함. 평가 데이터 형식 오류, 학습·평가·API의 입력 처리 불일치, 관련 테스트만 수정함. Ponytail 원칙에 따라 별도 처리기를 늘리지 않고 공통 함수로 모음. 소현이 담당 확장·프록시·CI 코드와 모델 가중치·Git 작업은 제외함.
- 백업: `.codex-backups/20260920-195911/`에 기존 대상 9개 파일을 상대 경로 그대로 복사하고 SHA-256 일치를 확인함. 기존 미커밋 변경도 포함해 보존함. 별도 `docs/CODEX_WORK_RULES.md`는 없어 `AGENTS.md` 절차를 따름.
- `services/api/app/ml/text.py`: 공통 `tokenize_article` 함수와 기본 길이 128을 추가함. 잔여 큰따옴표 이스케이프·공백 정리, 제목·본문 쌍 입력, longest_first 절단, token_type_ids 제거를 재사용함. `normalize=False`는 정리 전 대조 검사만을 위한 내부 옵션임.
- `services/api/app/ml/predictor.py`: 추론이 공통 입력 함수를 호출하도록 변경함. 정리는 실제 토큰화 시 한 번 수행함. 현재 모델 경로·식별자, 라벨 인덱스 조회, 128토큰 한도, 점수 반올림·5단계·표현 비교·API 계약은 유지함.
- `services/api/training/train_transformer.py`: 기존 tokenize_article를 공통 함수로 대체함. 정리되지 않은 기존 JSONL도 토큰화 시 정리함. 기본 max-length를 256에서 API와 같은 128로 변경하고, 새 학습의 저장 메타데이터에 정리·절단 규칙을 기록함. 명시적 --max-length 지정은 유지하므로 128 이외로 학습한 모델의 API 적용에는 길이 설정을 별도로 맞춰야 함. 이번에 재학습하거나 과거 모델의 메타데이터를 덮어쓰지 않음.
- `services/api/training/compare_models.py`: Transformer 비교 평가도 같은 입력 함수를 호출함. 보고서에 모델 경로, 정리·절단 규칙과 적용 길이 128을 기록함. scikit-learn 경로는 수정하지 않음. 기존 학습 당시 지표의 재현과 현재 서비스 입력 규칙으로 평가하는 것은 구분해야 함.
- `services/api/training/evaluate_quote_normalization.py`: 실제 로더가 반환하는 ArticleExample의 .title/.body/.label로 읽도록 수정함. 빈 평가 입력을 거절하고, before에만 normalize=False를 지정해 정리 전후 비교가 무효화되지 않게 함.
- `services/api/tests/test_quote_evaluation.py`: 가짜 dict 대신 ArticleExample을 사용함. 임시 JSONL을 실제 로더로 읽고 main/evaluate/보고서 저장까지 실행하는 검사, 기존 보고서 덮어쓰기 거절과 빈 입력 거절을 추가함. 이 단위 검사의 추론값만 대체함.
- `services/api/tests/test_predictor.py`, `services/api/tests/test_train_transformer.py`: 공통 함수 위임과 변경된 절단 계약에 맞춰 검사를 갱신함.
- 신규 `services/api/tests/test_model_inputs.py`: 실제 로컬 토크나이저로 학습 Dataset, 배치 비교 평가, API 추론이 동일한 유효 토큰을 받는지 검사함. 이스케이프·공백, 298자 제목, 긴 본문, 원문 대조 옵션, 기본 길이 일치를 포함함. 가중치는 필요 없지만 토크나이저 파일이 없으면 해당 두 검사는 생략됨. 이번 로컬 실행에서는 생략 없이 통과함.
- 검증: 전체 unittest 43개 모두 통과(실제 모델 API 검사 포함). 첫 실행은 Windows 격리 환경의 TemporaryDirectory 접근 거부로 신규 검사 1개가 실패했으며 코드 변경 없이 승인된 일반 권한으로 재실행해 모두 통과함. Starlette/httpx 사용 중단 예정 경고는 남음. `git diff --check` 통과.
- 실제 명령 검증: `python -B -X utf8 services/api/training/evaluate_quote_normalization.py --samples 2 --report-path C:/Users/Public/Documents/ESTsoft/CreatorTemp/truetitle-input-fix-20260920-195911.json`을 실제 기존 검증 JSONL·모델로 실행해 종료 코드 0과 보고서 생성을 확인함. 라벨별 1건의 실행 확인이며 표본 둘은 정리로 문자열이 바뀌지 않았음. 정확도 향상 증거나 모델 비교 결과로 해석하지 않음. 정리로 입력이 바뀌는 분기는 별도 실제 토크나이저·단위 검사로 확인함.
- 보존 확인: 현재 가중치 SHA-256 `b0a76937ed120bac8b268979493c679685ec93177c27830d17aa2b6a98e8ae6d`가 이전 기록과 같음. 기존 5만 건 모델도 보존함. 1만 건 모델의 최종 채택은 미확정이며 이번에 모델 재교체·서버 재시작·학습·업로드를 하지 않음.
- 영향과 미검증: 앞으로 실행하는 학습·평가의 입력 처리 규칙을 통일한 것이며, 이미 학습된 가중치를 소급 변경하는 작업은 아님. 128토큰 뒤 본문은 여전히 입력에 포함되지 않음. 독립 실전 정확도, 1만/5만 모델 우열, Chrome 통합 동작, OFF/초기화 응답 정리, 배포 프록시 신뢰 범위, 원격 CI는 미검증/별도 담당 과제로 남음.
- 복구: 이후 변경 여부를 확인하고 별도 승인 아래 위 백업의 9개 파일을 원래 경로로 복원하면 이번 수정 직전 상태가 됨. 신규 `services/api/tests/test_model_inputs.py`는 이번 작업에서 만든 파일임을 확인한 뒤 별도 승인으로 제거할 수 있음. 백업에는 이번 변경 기록의 직전 버전도 포함됨.

## 2026-09-20 - 입력 표기 민감도 개선 모델 연결

- 요청 및 승인: 사용자가 0점 쏠림과 따옴표 민감도 문제를 진단한 뒤 빠른 해결을 명시적으로 요청함. 정리된 데이터로 학습한 기존 모델 연결, 학습·추론 입력 정리 통일, 관련 테스트·문서·LFS 공유 설정을 수정함.
- 근거: 기존 5만 건 모델의 실제 기사 6건 × 4조건 비교에서, 본문 전체가 들어가는 두 기사도 따옴표 보정 유무에 따라 0점과 99점대로 갈렸음. 정리 데이터 1만 건 모델은 같은 첫 3건에서 표기 변화에 따른 차이가 작았고, 제목·본문을 교차한 합성 대조 입력 두 건에는 99.9점을 반환했음. 전체 실전 정확도가 개선됐다는 결론은 아님.
- 백업: `.codex-backups/20260920-191516/`에 기존 수정 대상 12개 파일을 원래 경로대로 복사하고 SHA-256 일치를 확인함. 기존 미커밋 문서 변경도 포함해 백업했으며 보존함. 모델 가중치는 덮어쓰지 않고 기존 경로에 그대로 보존함.
- `services/api/app/ml/predictor.py`: 기본 모델을 `transformer-10000-quote-normalized-v1`로 교체하고 식별자를 갱신함. 제목을 강제로 굽은 따옴표로 만드는 함수를 제거하고 제목·본문 모두 공통 입력 정리 함수를 호출함. 라벨 인덱스 조회·입력 128토큰·5구간·표현 비교·반환 형식은 유지함.
- `services/api/app/ml/text.py`: 기존 학습 전처리와 동일한 잔여 큰따옴표 이스케이프 및 연속 공백 정리 함수를 신규 분리함. 따옴표 모양·짝을 추정하거나 변경하지 않음. `services/api/training/prepare_part1.py`는 이 함수를 기존 `normalize_text` 이름으로 가져와 재사용함.
- `services/api/training/evaluate_quote_normalization.py`: 변경된 API 입력 처리에 맞춰 제목·본문 모두의 정리 전후를 비교하고 본문 변경 건수도 기록하도록 갱신함.
- 테스트 3개 파일: `test_predictor.py`는 잔여 이스케이프 제거·홀수 인용부호 보존·양쪽 모델 입력 정리를 확인함. `test_main.py`는 동일 이스케이프 입력의 응답 일치, 실제 모델의 따옴표 형태 안정성 및 무관한 본문 구분을 확인함. `test_quote_evaluation.py`는 제목 또는 본문만 변경되는 경우의 개선·회귀 집계를 확인함.
- `.gitignore`, `.gitattributes`: 새 런타임 모델 폴더 5개 파일을 공유 대상으로 허용하고 `model.safetensors`를 Git LFS 대상으로 지정함. 새 가중치 272,382,200바이트, SHA-256 `b0a76937ed120bac8b268979493c679685ec93177c27830d17aa2b6a98e8ae6d`. 이번에 재학습하거나 모델 파일을 생성·다운로드·업로드하지 않음.
- 문서: README에 새 모델 경로·식별자·학습 1만/검증 2천/2에포크/저장 정확도 95.60%와 재시작·모델 수신 절차를 반영함. `docs/model-evaluation.md`는 교체 근거와 남은 한계를 추가하고 기존 5만 건 기록을 과거 자료로 구분함. `docs/test.md`의 현재 모델 경로를 갱신함.
- 검증: API unittest 38개 모두 통과(실제 모델 테스트 포함). 전처리·입력 비교 스크립트의 `--help` 직접 실행 통과. `git diff --check` 통과, 새 모델의 filter/diff/merge가 모두 lfs임을 확인함. Git 상태에서 새 런타임 모델 파일 5개가 미추적 커밋 후보로 나타남을 확인함. 사용 중단 예정 Starlette/httpx 경고는 있으나 테스트 실패는 없음.
- 영향과 미검증: API 서버 재시작 후 새 모델이 로드됨. 이미 실행 중인 서버는 자동 교체되지 않음. 새 모델 파일 없이 코드만 받으면 503이므로 함께 커밋·push해야 함. 실제 Chrome 재시작 후 통합 결과, 독립적인 실제 기사 정확도, 과장·호기심 유발 유형의 전반적 개선은 미검증. 새 모델도 해당 유형을 낮게 판단하는 사례가 있어 해결 완료로 주장하지 않음. 확장 코드·서버 공통 파일·Git commit/push/merge는 변경하지 않음.
- 복구: 이후 변경을 확인하고 위 백업의 12개 파일을 복원하면 이전 5만 건 모델 경로·입력 보정으로 돌아갈 수 있음. 신규 `app/ml/text.py`는 복구 후 참조가 없는지 확인한 뒤 별도 승인으로 제거할 수 있음. 이전·새 모델 가중치는 모두 보존되어 있음.

## 2026-09-20 - 마무리 점검과 확장 초기 기록 구분

- 요청 및 승인: 사용자가 프로젝트 마무리 점검 후 다음 작업 진행을 승인함. 현재 문서와 예전 설명을 구분하고, OFF·초기화 중 분석 결과가 도착하는 상황을 점검함.
- 백업: `.codex-backups/20260920-184328/`에 `docs/test.md`, 이 변경 기록을 원래 경로로 복사하고 각 파일의 SHA-256 일치를 확인함. 별도 `docs/CODEX_WORK_RULES.md`는 없어 `AGENTS.md` 절차를 적용함.
- 수정 파일: `docs/test.md` 앞에 현재 지원 범위·실제 모델 연결 상태·최종 점검표와 설정 변경 중 응답 처리 재현 결과를 추가함. 기존 1·2차 구현 설명은 과거 기록임을 명시하고 보존함. 이 파일에는 요청·검증·복구 방법을 기록함.
- 이유와 영향: 예전의 mock 및 모델 부재 설명을 현재 상태로 오해하지 않도록 정리함. 실행 코드·모델·의존성은 변경하지 않으며 팀원의 미게시 수정도 대신 구현하지 않음.
- 검증: `git ls-remote --heads origin`으로 원격 main이 로컬과 동일한 `039c3430b945d8513413eb774236e9a71c5e42a7`임을 확인함. `feat/extension-bootstrap`은 `a02d93e`이며 현재 로컬 소스에 다음 지원·홀수 큰따옴표 보호·프록시 헤더 변경이 없음을 확인함. 바로 전 점검에서 같은 커밋의 API unittest 36개 및 TypeScript 검사가 통과했으며 이번에는 실행 코드가 바뀌지 않아 재실행하지 않음.
- 재현: 저장소 밖 `C:/Users/Public/Documents/ESTsoft/CreatorTemp/truetitle-settings-check.cjs`에서 실제 settings·background·content TypeScript를 변환해 실행함. Chrome storage·DOM·API 응답만 대체했으며 OFF 및 초기화 후 이전 응답이 result로 표시되는 현상 두 건을 재현함. 두 경우 모두 이후 새 분석 호출은 차단됨을 assert로 확인함. 네트워크·실제 브라우저 설정은 변경하지 않음.
- 문서 검증: README·평가 기록 링크 대상 존재와 변경 파일이 문서 두 개뿐임을 확인함. `git diff --check` 통과. Git의 LF→CRLF 안내는 있었으나 공백 오류는 없었음.
- 미검증: 실제 Chrome에서의 설정 변경 경쟁 상황, 팝업 연속 요청, 별도 환경 설치·실행은 아직 미검증. 팀원의 로컬 미게시 작업은 확인하지 못함. Git commit·push·병합·브랜치 변경은 수행하지 않음.
- 원상복구: 이후 변경 유무를 확인하고 위 백업의 두 문서를 원래 위치에 복원하면 됨. 임시 재현 파일은 저장소 밖에 있어 커밋 대상이 아님.
