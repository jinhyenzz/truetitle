"""같은 검증 기사에 API의 따옴표 보정 전후를 적용해 회귀를 확인한다."""
...
from app.ml.predictor import MODEL_PATH, normalize_straight_quotes, predict_clickbait_score
from training.compare_models import DEFAULT_INPUT_DIR, calculate_metrics, load_examples


def evaluate(examples: list[dict]) -> dict:
    labels, before, after = [], [], []
    changed_titles = changed_predictions = improved = regressed = 0
    score_deltas = []
    started = time.perf_counter()
    for index, article in enumerate(examples, start=1):
        title, body, label = article["title"], article["body"], article["label"]
        normalized = normalize_straight_quotes(title)
        old_score = predict_clickbait_score(title, body)
        new_score = old_score if normalized == title else predict_clickbait_score(normalized, body)
...
def main() -> None:
    ...
    report = {
        ...
        "normalization": "paired double quotes in title only",
